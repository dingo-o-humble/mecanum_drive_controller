  ...
  const rclcpp::Time & time, const rclcpp::Duration & period)
{
  ...
  // A temporary workaround for the bug that if teleop is already on
  // then odometry position shoots up impossible high velue as velocity value is high
  const double max_possible_velocity = 100;
  ...
  if (
    ...
    wheel_front_left_vel < max_possible_velocity && wheel_back_left_vel < max_possible_velocity &&
    wheel_back_right_vel < max_possible_velocity && wheel_front_right_vel < max_possible_velocity)
  {
    ...
    !std::isnan(reference_interfaces_[2]))
  {
    ...
    // Calculate the maximum absolute value among the inputs
    double max_value = std::max({
        std::abs(w_front_left_vel),
        std::abs(w_back_left_vel),
        std::abs(w_back_right_vel),
        std::abs(w_front_right_vel)
    });

    // max velocity for the motors, Todo: @skawar1305 get as a parameter
    const double limit = 10;

    // Check if the max value exceeds the limit
    if (max_value >= limit) {
      // Calculate the scaling factor
      double scaling_factor = limit / max_value;

      // Scale each value
      w_front_left_vel *= scaling_factor;
      w_back_left_vel *= scaling_factor;
      w_back_right_vel *= scaling_factor;
      w_front_right_vel *= scaling_factor;
    }
